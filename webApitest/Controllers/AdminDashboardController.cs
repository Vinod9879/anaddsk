...
namespace webApitest.Controllers
{
    ...
    public class AdminDashboardController : ControllerBase
    {
        ...
        public async Task<IActionResult> GetDocumentDetails(int documentId)
        {
            try
            {
                ...
                return Ok(new
                {
                    ...
                    ecPath = document.ECPath,
                    aadhaarPath = document.AadhaarPath,
                    panPath = document.PANPath,
                    uploadedAt = document.CreatedAt,
                    hasExtractedData = !string.IsNullOrEmpty(document.ExtractedData),
            ...
            {
                _logger.LogError(ex, $"Error getting document details for ID: {documentId}");
                return StatusCode(500, "Internal server error");
            }
        }
        ...
        [Authorize]
        public async Task<IActionResult> DownloadDocument(int documentId, string documentType)
        {
            try
            {
                var document = await _context.UserUploadedDocuments
                ...
                var fileName = $"{documentType}_{documentId}_{Path.GetFileName(filePath)}";
                
                return File(fileBytes, "application/pdf", fileName);
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, $"Error downloading document {documentType} for ID: {documentId}");
                return StatusCode(500, "Internal server error");
            }
        }
