...
const AdminDashboard = () => {
  ...
  const handleTriggerVerification = async (documentId) => {
    try {
      ...
      setVerifyingDocuments(prev => new Set(prev).add(documentId));
      setMessage('Verifying documents...');
      
      console.log('Calling documentService.triggerVerification...');
                      ...
                      filteredDocuments.map((doc) => (
                        <tr key={doc.id}>
                          ...
                          <td>
                           ...
                           <td>
                             <div className="d-flex flex-wrap gap-1">
                               {/* Step 1: Extract Button - Only visible when document is uploaded but not extracted */}
                               {!doc.hasExtractedData && (
                                 <Button 
                                   variant="outline-success" 
                                   size="sm"
                                   onClick={() => handleExtractDocuments(doc.id)}
                                   disabled={extractingDocuments.has(doc.id)}
                                   title={extractingDocuments.has(doc.id) ? "Extracting..." : "Extract data from documents"}
                                 >
                               ...
                               {/* Step 2: View Button - Always visible after extraction */}
                               {doc.hasExtractedData && (
                                 <Button 
                               ...
                               {/* Step 3: Verify Button - Show when extracted and not yet verified */}
                               {doc.hasExtractedData && 
                                !doc.IsVerified && 
                                doc.VerificationStatus !== 'Verified' && 
                                doc.VerificationStatus !== 'Rejected' && 
                                doc.VerificationStatus !== 'Under Review' && 
                                !verifyingDocuments.has(doc.id) && (
                                 <Button 
                                   variant="outline-primary" 
                                   size="sm"
                                   onClick={() => {
                                     console.log('Verify button clicked for document:', doc);
                                     handleTriggerVerification(doc.id);
                                   }}
                               ...
                               {/* Verify Button - Loading state when verification is in progress */}
                               {doc.hasExtractedData && verifyingDocuments.has(doc.id) && (
                                 <Button 
                               ...
                               {/* Step 4: Results Button - Show after extraction (always visible after extraction) */}
                               {doc.hasExtractedData && (
                                 <Button 
                                   variant="outline-info" 
                                   size="sm"
                                   onClick={() => {
                                     console.log('Results button clicked for document:', doc.id);
                                     handleViewVerificationResults(doc.id);
                                   }}
                               ...
                               {/* Location Button - Show after extraction (always visible after extraction) */}
                               {doc.hasExtractedData && (
                                 <Button 
                               ...
                               {/* Status Indicators - Show current verification status */}
                               {doc.VerificationStatus === 'Verified' && (
                                 <Button 
                                   variant="success" 
                                   size="sm"
